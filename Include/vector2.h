...
template <typename T> struct TVec2 {
  ...
  inline TVec2<T>() : x(T(0)), y(T(0)) {}

  inline TVec2<T>(T _x, T _y) : x(_x), y(_y) {}

  T &operator[](int i) { return ((&x)[i]); }

  const T &operator[](int i) const { return ((&x)[i]); }

  TVec2<T> operator*(float s) { return TVec2<T>{x * s, y * s}; }
...
inline FVec2 operator-(const FVec2 &v) { return (FVec2(-v.x, -v.y)); }

template <typename T> inline float Magnitude(const TVec2<T> &v) {
